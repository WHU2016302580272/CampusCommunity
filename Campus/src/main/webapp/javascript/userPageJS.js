...
function getData(arr) {//从数据arr[i][4] = obj[i].date;库得到数据
    ...
    switch (page) {
        case 'star':
            records.length=records.length*2;
            for (var i = 0; i < arr.length; i++) {
                records[i*2] = arr[i][1];
                records[i*2+1]=arr[i][0];
            }
            break;
        case 'que':
            for (var i = 0; i < arr.length; i++) {
...
function updateStar() {
    ...
    for (var j = firstRe; j < Math.min(reNum, limit * pageNum); j++) {
        ...
        conO.innerHTML += "<a style='text-decoration:none' href=passage.jsp?pid=" + records[j*2+1] + " target='_blank'>" +
            "<p class='starTxt'>" + records[j*2] + "</p></a>";//插入数据
    }
