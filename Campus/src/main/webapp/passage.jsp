...
<body onload="print(),commentPrint(),logined(),getIfFavorite()">
...
<div  align="right" id ="back_div">
    <a id="back"  href="homePage.jsp">返回主页</a>
</div>
...
<div id="main_question">
    <div id="main_passage">
        ...
        <table width="100%">
            <tr>
                <%--放多有多少个回答--%>
                <td width="20%"><a id="how_many_answer" class="how_many">多少个回答</a></td>
                <%--放多有多少个收藏--%>
                <td width="20%"><a id="how_many_collection" class="how_many"></a></td>
                <td align="right">
                    <%--收藏的IMG 点了要把这个页加到用户的收藏目前collection()只有換图片--%>
                    <img src="/resource/icon_mark.png" id="collection"  onclick="favorite()" title="收藏">
                </td>
                <%--点了跳出一个回答的DIV块--%>
    ...
    <div id="owner_msg_div">
        <div id="owner">
            ...
            <div id="user_img">
                <figure>
                    <%--//放icon--%>
                    <div id="owner_circle"><img id="img" src="/resource/icon.jpg" onclick=location="userPage.jsp">
                    </div>
                </figure>
                <%--//放名字--%>
...
<script>
    ...
    function toArray(json) {
        ...
        for (var i = 0; i < obj.length; i++) {
            ...
            arr[i][7] = "data:" + obj[i].avatar;
            arr[i][8] = obj[i].hasFavorite;
            arr[i][9] = obj[i].uid;
        }
        return arr;
    }
    ...
    //收藏
    function getIfFavorite() {
        var url = "/IfFavoriteServlet";
        var pidd = <%=pid%>
        $.ajax({
            type: "GET",
            url: url,
    ...
    function favorite() {
        var url = "/FavoritePassageServlet";
        var pidd = <%=pid%>
        var hasFavorite = collectCondition;
            $.ajax({
                type: "POST",
                url: url,
                data: {
