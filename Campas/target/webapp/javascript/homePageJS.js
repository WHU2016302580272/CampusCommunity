...
function test(arr) {
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        //div3_i_PASSAGE的css
        var d = 'width:70%;height:90px;position:absolute;background:#ffffff;left:170px;top:' + d_y + 'px;text-align:left;line-height:90px;'
        var url;
        ...
        f += '<div class=div3_' + i + ' id=div3_' + i + ' style=width:100%;height:90px;  ><div class=div3_i_user_img style=' + a + '  ><figure><div class=circle><img id=img' + i + '  class=img src="/resource/icon.jpg" onclick=location="otherUserPage.jsp?uid='+arr[i][6]+'"></div></figure><div  class=div3_' + i + '_user_msg style=' + b + '><a id=school' + i + ' class=msg  href=otherUserPage.jsp?uid='+arr[i][6]+'>大学</a><br><a id=collage' + i + ' class=msg href=otherUserPage.jsp?uid='+arr[i][6]+'>学院</a><br><a id=year' + i + ' class=msg href=otherUserPage.jsp?uid='+arr[i][6]+'>年级</a></div><div id=div3_' + i + '_rectangle style=' + c + ' class=div3_rectangle_border></div></div ><div id=div3_' + i + '_passage class=div3_passage_border style=' + d + ' ><a id=pas_title' + i + ' class=pas   href=passage.jsp?pid='+arr[i][5] +'>url</a></div></div>';
    }
    //把f加到html中的div3的这个块
    ...
    document.getElementById("div3").innerHTML =h + g + f + '<div id=div7>' +
        '<table id=pre_next ><tr><td align=right><input id=next type=button value=上一页  onclick=passagedown()></td><td align=right><input id=pre type=button value=下一页 onclick=passageup() ></td></table></div>';
    //改內容
    for (var i = 0; i < arr.length; i++) {
        if (arr[i][2] == "null") {
            break;
